...
public class ThirdActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        imgBtnPhone.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if(phoneNumber == null || phoneNumber.isEmpty()) {
                    ...
                    //Comprobar version actual de android que estamos usando
                    if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {

                        //Comprobar si ha aceptado, no ha aceptado o nunca se ha preguntado
                        if (CheckPermission(CALL_PHONE)) {
                            //Ha aceptado
                            Intent i = new Intent(Intent.ACTION_CALL, Uri.parse("tel:" + phoneNumber));
                            if (ActivityCompat.checkSelfPermission(ThirdActivity.this, CALL_PHONE) != PackageManager.PERMISSION_GRANTED)
                                return;
                            startActivity(i);
                        } else {
                            //O no ha aceptado o es la primera vez que se le preguntas
                            if(!shouldShowRequestPermissionRationale(CALL_PHONE)){
                                //No se ha preguntado aun
                                requestPermissions(new String[]{CALL_PHONE}, PHONE_CALL_CODE);
                            }else{
                                //Denegado
                                Toast.makeText(ThirdActivity.this, "Enable the request permission", Toast.LENGTH_LONG).show() ;
        ...
        imgBtnWeb.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                //Email para el ejemplo del Intent con email
                String email = "alejandro" ;
                if(url != null && !url.isEmpty()){
                    //Otra forma de declarar los Intent
                    Intent intentWeb = new Intent() ;
                    intentWeb.setAction(Intent.ACTION_VIEW);
                    intentWeb.setData(Uri.parse("http://"+url)) ;

                    //Ejemplo Intent para que abra los contactos
                    ...
                    //Así lanzamos siempre el selector de aplicaciones, elegimos el intent por el que
                    //queremos preguntar  y un simple título
                    //startActivity(Intent.createChooser(intentMail, "Elige cliente de correo"));

                    //Telefono segundo caso para no llamar directamente y sin necesitar permisos
                    Intent intentPhone = new Intent(Intent.ACTION_DIAL, Uri.parse("tel:666544456")) ;
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        //Estamos en el caso de pedir permisos para el teléfono
        switch(requestCode) {
            case PHONE_CALL_CODE:
                ...
                if(permission.equals(Manifest.permission.CALL_PHONE)){
                    ...
                    //Comprobar si ha sido aceptado o denegado la petición de permisos
                    if(result == PackageManager.PERMISSION_GRANTED){
                        //Concedio el permiso
                        String phoneNumber = editTextPhone.getText().toString() ;
                        Intent intentCall = new Intent(Intent.ACTION_CALL, Uri.parse("tel:"+phoneNumber));
                        if(ActivityCompat.checkSelfPermission(this, CALL_PHONE) != PackageManager.PERMISSION_GRANTED) return ;
                        startActivity(intentCall);
                    }else{
